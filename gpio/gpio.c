...
static void doExports (int argc, char *argv [])
{
  ...
  for (first = 0, i = 0 ; i < 32 ; ++i)	// Crude, but effective  
  { 
...
static void doPwmMode_bal (int argc, char *argv [])
{
  unsigned int channel;
  unsigned int mode = PWM_MODE_BAL;

  if(BPVER == piBoardRev())
  {
	if (argc != 3)
    {
      fprintf (stderr, "Usage: %s pwm-bal <channel>\n", argv [0]) ;
      exit (1) ;
    }

  	//PWM1->channel 1, PWM_2->channel 2
	channel = (unsigned int)strtoul (argv [2], NULL, 10) ;
	if(channel == 1 || channel == 2)
	{
	  printf ("pwm channel set to %d\n", channel) ;
	}
	else
	{
	  fprintf (stderr, "%s: channel must be set to 1 or 2\n", argv [0]) ;
      exit (1) ;
	}

	sunxi_pwm_set_mode (mode, channel) ;
  } 
  else 
  {
    pwmSetMode (mode) ; 
...
static void doPwmMode_ms (int argc, char *argv [])
{
  unsigned int channel;
  unsigned int mode = PWM_MODE_MS;

  if(BPVER == piBoardRev())
  {
	if (argc != 3)
    {
      fprintf (stderr, "Usage: %s pwm-ms <channel>\n", argv [0]) ;
      exit (1) ;
    }
...
static void doPwmRange (int argc, char *argv [])
{
  ...
  if(BPVER == piBoardRev())
  {
    if (argc != 4)
    {
      fprintf (stderr, "Usage: %s pwmr <channel> <range>\n", argv [0]) ;
      exit (1) ;
    }
	...
	{
	  fprintf (stderr, "%s: channel must be set 1 or 2\n", argv [0]) ;
      exit (1) ;
	}
	...
	sunxi_pwm_set_period(range, channel) ;
  } 
  else 
  {
    if (argc != 3)
...
static void doPwmClock (int argc, char *argv [])
{
  ...
  if(BPVER == piBoardRev())
  {
	if (argc != 4)
    {
      fprintf (stderr, "Usage: %s pwmc <channel> <clock>\n", argv [0]) ;
      exit (1) ;
    }
	...
	sunxi_pwm_set_clk (clock, channel) ;
  } 
  else 
  {
    if (argc != 3)
...
// Pi Specifics
  ...
  else if (strcasecmp (argv [1], "pwm-bal"  ) == 0) doPwmMode_bal    (argc, argv) ;
  else if (strcasecmp (argv [1], "pwm-ms"   ) == 0) doPwmMode_ms    (argc, argv) ;
