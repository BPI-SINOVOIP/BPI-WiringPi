  ...
  args1 [0] = args2 [0] = 0 ;
	...
	if(BPVER ==  piBoardRev())
	{
	  if (strcasecmp (argv [2], "spi") == 0)
	  {
		...
		//bpi, spi driver is built-in
		fprintf (stderr, "spi driver is built-in, no more loading\n") ;
	  ...
	  else if (strcasecmp (argv [2], "i2c") == 0)
	  {
		...
		//bpi, spi driver is built-in
		fprintf (stderr, "i2c driver is built-in, no more loading\n") ;
		changeOwner (argv [0], file1) ;
...
static void doUnLoad (int argc, char *argv [])
{
  ...
  if(BPVER ==  piBoardRev())
  {
	if (strcasecmp (argv [2], "spi") == 0)
    {
	  ...
	  //bpi, spi driver is built-in
	  fprintf (stderr, "spi driver is built-in, no more unloading\n") ;
	  exit (1) ;
    }
    else if (strcasecmp (argv [2], "i2c") == 0)
    {
	  ...
	  //bpi, spi driver is built-in
	  fprintf (stderr, "i2c driver is built-in, no more unloading\n") ;
	  exit (1) ;
    }
    else
      _doUnLoadUsage (argv) ;
...
static void doI2Cdetect (int argc, char *argv [])
{
  ...
  if(BPVER == piBoardRev())
  {
	...
	sprintf (command, "%s -y %d", I2CDETECT, 1) ;
	if (system (command) < 0)
      fprintf (stderr, "%s: Unable to run i2cdetect: %s\n", argv [0], strerror (errno)) ;
  }
  else 
...
void doExport (int argc, char *argv [])
{
  ...
  if (argc != 4)
  {
    fprintf (stderr, "Usage: %s export pin mode, pin number is BP, not WPI\n", argv [0]) ;
    exit (1) ;
  }
...
void doMode (int argc, char *argv [])
{
  ...
  else if (strcasecmp (mode, "i2c")     == 0) pinMode         (pin, I2C_PIN) ;
  else if (strcasecmp (mode, "spi")     == 0) pinMode         (pin, SPI_PIN) ;
